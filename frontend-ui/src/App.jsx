 ...
 * ===============================================
 * Boot sequence:
 * 1. DB initialises & Pre-fetches Configuration →
 * 2. SplashScreen visible (exactly 15 seconds) →
 * 3. First-run setup wizard (if extension ID is absent) →
 * 4. Main Dashboard Engine Container
 */
...
function SetupScreen({ onComplete }) {
  ...
  const handleFinish = async () => {
    if (!extId.trim()) { 
      setError('Paste your Extension ID to continue.'); 
      return; 
    }
...
export default function App() {
  ...
  // Initialize operational settings from IndexedDB store during initial lifecycle paint
  useEffect(() => {
    async function initConfigStore() {
      try {
        const done = await getSetting('setupDone', false);
        setIsSetupDone(!!done);
      } catch (err) {
        console.error("Initialization error checking configuration store settings:", err);
      } finally {
        setAppState(STATE.SPLASH);
      }
    }
    initConfigStore();
  }, []);
  ...
  const handleSplashComplete = () => {
    setAppState(isSetupDone ? STATE.APP : STATE.SETUP);
  };
  ...
  if (appState === STATE.LOADING) {
    return (
      <div style={{
        ...
        <div style={{ color: '#3b82f6', fontSize: 28, animation: 'pulse 1.5s infinite' }}>⚡</div>
      </div>
    );
  }

  return (
    <div style={{ background: '#0a0f1e', minHeight: '100vh', width: '100vw', overflow: 'hidden' }}>
      {/* ── Phase 1: Splash Sequence ── */}
      {appState === STATE.SPLASH && (
        <SplashScreen onComplete={handleSplashComplete} />
      )}

      {/* ── Phase 2: First-Time Extension Environment Mapping ── */}
      {appState === STATE.SETUP && (
        <SetupScreen onComplete={handleSetupComplete} />
      )}

      {/* ── Phase 3: Core Desktop Application Layer ── */}
      {appState === STATE.APP && (
        <Dashboard />
      )}
    </div>
...
// ── Setup Screen Layout Blueprint Styles ──────────────────────────────────────
const stp = {
  ...
  sub:       { fontSize: 13, color: '#94a3b8', margin: '6px 0 2px' }, // Upgraded color contrast accessibility parameter
  sub2:      { fontSize: 11, color: '#475569', margin: '0 0 28px', fontFamily: 'monospace' }, // Upgraded color contrast accessibility parameter
  ...
  btnSkip:   { padding: '12px 20px', background: 'rgba(255,255,255,0.04)', color: '#94a3b8', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 8, fontSize: 13, cursor: 'pointer' }, // Upgraded color contrast accessibility parameter
  footer:    { marginTop: 20, fontSize: 10, color: '#38bdf8', letterSpacing: '0.5px' }, // Boosted readability accent
};